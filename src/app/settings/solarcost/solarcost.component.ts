...
import { Solarcost, PlantNam } from '../Data';
...
@Component({
  selector: 'app-solarcost',
  templateUrl: './solarcost.component.html',
  styleUrls: ['./solarcost.component.css'],
  providers:[DatePipe]
})
export class SolarcostComponent {
  ...
  endpoint = 'costdata'
  sourceName = 'SOLAR'
  url = '6465d7719c09d77a62f568cc.mockapi.io'

  GetData(){
    this.ApiData.GetCostData(this.endpoint, this.sourceName).subscribe(
      response => {
        this.SolCostData = response
        this.load = false
        this.loading = false
      })
  }
  ...
  SavaData(newData:any){
    this.SolCostData.push(newData)
  }

  Update(Update:any){
    const i = this.SolCostData.findIndex((sol) => sol.solar_id === this.selectedData.solar_id);
    this.SolCostData[i] = Update;
  }
  ...
  constructor(private ApiData: SettingsService, private messageService: MessageService, private confirmation: ConfirmationService, private Cookie: CookieService, private datepipe: DatePipe) {
    this.form = new FormGroup({
      solar_id: new FormControl(''),
      solar_installedCap: new FormControl('', Validators.required),
      solar_cst: new FormControl('', Validators.required),
    });
  }
  ...
  deleteselectedRows() {
    this.confirmation.confirm({
      message: 'Sure, you want to delete the selected Solar cost?',
  ...
  DeleteData(){
    ...
    {
      const id = this.selected[i].solar_id;
      this.ApiData.DeleteCostData(this.endpoint, this.sourceName, id).subscribe(
        () => {
          const index = this.SolCostData.findIndex(j => j.solar_id === id);
          if (index !== -1) {
            this.maskLoad = false
            this.SolCostData.splice(index, 1);
  ...
  exportExcel(){
    this.maskLoad = true
    const tableElement = document.getElementById('exportsortable');
    const sheetname = 'sheet1';
    const wb = XLSX.utils.book_new();
    const tabledata = XLSX.utils.table_to_sheet(tableElement);
    const sheetData: { [key: string]: any }[] = XLSX.utils.sheet_to_json(tabledata);
    // for (const cell of sheetData) {
    //   cell['DATE'] = this.datepipe.transform(cell['DATE'], 'dd-MM-yyyy')
    // }
    const numRows = sheetData.length;
    let Heading = [[`SOLAR COST DETAILS FOR ${this.Cookie.get('plantname').toUpperCase()}`]];
    const merge = [
      { s: { r: 1, c: 1 }, e: { r: 1, c: 2 }  },
    ];
    ...
    ws['!cols'] = [{ width: 10 }, { width: 25 }, { width: 20 }];

    XLSX.utils.book_append_sheet(wb, ws, sheetname);
    XLSX.writeFile(wb, `Solar Cost Details.xlsx`);
    this.maskLoad = false
  }
