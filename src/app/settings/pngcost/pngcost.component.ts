...
import { PngCost } from '../Data';
import * as XLSX from 'xlsx-js-style'
import { DatePipe } from '@angular/common';
import { CookieService } from 'ngx-cookie-service';

@Component({
  selector: 'app-pngcost',
  templateUrl: './pngcost.component.html',
  styleUrls: ['./pngcost.component.css'],
  providers:[DatePipe]
})
export class PngcostComponent {
  ...
  GetData(month:string, year:string){
    this.ApiData.GetCostDataMonth(this.endpoint, this.SourceName, month, year).subscribe(
      response => {
        this.PngcostData = response
        // console.log(this.PngcostData)
        this.load = false
        this.loading = false
      })
  }
  ...
  SavaData(newData:any){
    this.PngcostData.push(newData)
  }

  Update(Update:any){
    const i = this.PngcostData.findIndex((wind) => wind.PNG_id === this.selectedData.PNG_id);
    this.PngcostData[i] = Update;
  }
  ...
  constructor(private ApiData: SettingsService, private messageService: MessageService, private confirmation: ConfirmationService, private datepipe: DatePipe, private Cookie: CookieService) {
    this.form = new FormGroup({
      PNG_id: new FormControl(''),
      PNG_date: new FormControl('', Validators.required),
      PNG_kg_cons: new FormControl('', Validators.required),
      PNG_cpkg: new FormControl('', Validators.required),
    });
  ...
  DeleteData(){
    ...
    {
      const id = this.selected[i].PNG_id;
      this.ApiData.DeleteCostData(this.endpoint, this.SourceName, id).subscribe(
        () => {
          const index = this.PngcostData.findIndex(j => j.PNG_id === id);
          if (index !== -1) {
            this.maskLoad = false
            this.PngcostData.splice(index, 1);
