...
import { CngCost } from '../Data';
import * as XLSX from 'xlsx-js-style'
import { DatePipe } from '@angular/common';
import { CookieService } from 'ngx-cookie-service';

@Component({
  selector: 'app-cngcost',
  templateUrl: './cngcost.component.html',
  styleUrls: ['./cngcost.component.css'],
  providers: [DatePipe]
})
export class CngcostComponent implements OnInit {
  ...
  SavaData(newData:any){
    this.CngcostData.push(newData)
  }

  Update(Update:any){
    const i = this.CngcostData.findIndex((wind) => wind.CNG_id === this.selectedData.CNG_id);
    this.CngcostData[i] = Update;
  }
  ...
  constructor(private ApiData: SettingsService, private messageService: MessageService, private confirmation: ConfirmationService, private datepipe: DatePipe, private Cookie: CookieService) {
    this.form = new FormGroup({
      CNG_id: new FormControl(''),
      CNG_date: new FormControl('', Validators.required),
      CNG_kg_cons: new FormControl('', Validators.required),
      CNG_cpkg: new FormControl('', Validators.required),
    });
  ...
  deleteselectedRows() {
    this.confirmation.confirm({
      message: 'Sure, you want to delete the selected CNG cost?',
  ...
  DeleteData(){
    ...
    {
      const id = this.selected[i].CNG_id;
      this.ApiData.DeleteCostData(this.endpoint, this.SourceName, id).subscribe(
        () => {
          const index = this.CngcostData.findIndex(j => j.CNG_id === id);
          if (index !== -1) {
            this.maskLoad = false
            this.CngcostData.splice(index, 1);
  ...
  exportExcel(){
    this.maskLoad = true
    const tableElement = document.getElementById('exportcngtable');
    ...
    let Heading = [[`CNG COST DETAILS FOR ${this.Cookie.get('plantname').toUpperCase()}`]];
    const merge = [
      { s: { r: 1, c: 1 }, e: { r: 1, c: 3 }  },
    ];
    ...
    XLSX.writeFile(wb, `CNG Cost Details.xlsx`);
    this.maskLoad = false
  }
