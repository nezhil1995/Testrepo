...
export class HolidaySettingsComponent {
  ...
  OnDateChanges(){
    let dateValue = this.form.get('hd_date')?.value
    let day = this.datepipe.transform(dateValue, 'EEEE')
    this.form.controls['hd_day'].setValue(day)
  }
  ...
  GetData(month:string, year:string){
    this.ApiData.GetCostDataMonth(this.endpoint, this.SourceName, month, year).subscribe(
      response => {
        ...
        this.form.controls['hd_plantname'].setValue(this.Cookie.get('plantname'))
        this.platname = this.Cookie.get('plantname')
  ...
  SavaData(newData:any){
    this.Holidaydata.push(newData)
  }

  Update(Update:any){
    const i = this.Holidaydata.findIndex((item) => item.hd_id === this.selectedData.hd_id);
    this.Holidaydata[i] = Update;
  }
  ...
  constructor(private ApiData: SettingsService, private messageService: MessageService, private confirmation: ConfirmationService, private datepipe: DatePipe, private Cookie: CookieService,private route: ActivatedRoute) {
    this.form = new FormGroup({
      hd_id: new FormControl(''),
      hd_date: new FormControl('', Validators.required),
      hd_day: new FormControl('', Validators.required),
      hd_description: new FormControl('', Validators.required),
      hd_plantname: new FormControl(''),
    });
  ...
  deleteselectedRows() {
    this.confirmation.confirm({
      message: 'Sure, you want to delete the selected PNG cost?',
  ...
  DeleteData(){
    ...
    {
      const id = this.selected[i].hd_id;
      this.ApiData.DeleteData(this.endpoint, id).subscribe(
        () => {
          const index = this.Holidaydata.findIndex(j => j.hd_id === id);
          if (index !== -1) {
            this.maskLoad = false
  ...
  exportExcel(){
    this.maskLoad = true
    const tableElement = document.getElementById('exportpngtable');
    ...
    let Heading = [[`PNG COST DETAILS FOR ${this.Cookie.get('plantname').toUpperCase()}`]];
    const merge = [
      { s: { r: 1, c: 1 }, e: { r: 1, c: 3 }  },
    ];
    ...
    XLSX.writeFile(wb, `PNG Cost Details.xlsx`);
    this.maskLoad = false
  }
